...
class ConstrainedGail:
    ...
    def __init__(
        ...
        *,
        ...
        self.n_constraints = constraints.shape[0]

        discrim_kwargs = discrim_kwargs or {}
        discrim_expert = discrim_nets.DiscrimNetGAIL(
            venv.observation_space, venv.action_space, **discrim_kwargs
        )

        discrim_constraints = []
        for _ in range(self.n_constraints):
            discrim_constraints.append(
                discrim_nets.DiscrimNetGAIL(
                    venv.observation_space, venv.action_space, **discrim_kwargs
                )
            )
        ...
        self.constraint_data_loader = []
        self._endless_constraint_iterator = []
        for i in range(self.n_constraints):
            if isinstance(constraints[i], types.Transitions):
                if len(constraints[i]) < expert_batch_size:
                    raise ValueError(
                        "Provided Transitions instance as `constraint[" + str(i) +"]` argument but "
                        "len(constraints[" + str(i) + "]) < expert_batch_size. "
                        f"({len(constraints[i])} < {expert_batch_size})."
                    )
                self.constraint_data_loader.append(
                    th_data.DataLoader(
                        constraints[i],
                        batch_size=expert_batch_size,
                        collate_fn=types.transitions_collate_fn,
                ...
                self.constraint_data_loader.append(constraints[i])
            self._endless_constraint_iterator.append(util.endless_iter(self.constraint_data_loader[-1]))
        ...
        self.discrim_expert = discrim_expert.to(self.gen_algo.device)
        self._disc_opt_cls = disc_opt_cls
        self._disc_opt_kwargs = disc_opt_kwargs or {}
        self._init_tensorboard = init_tensorboard
        self._init_tensorboard_graph = init_tensorboard_graph
        self._disc_opt = self._disc_opt_cls(
            self.discrim_expert.parameters(), **self._disc_opt_kwargs
        )
        ...
        if debug_use_ground_truth:
            ...
            self.venv_wrapped = reward_wrapper.RewardVecEnvWrapper(
                self.venv_norm_obs, self.discrim.predict_reward_train #################Change this###################
            )
    ...
    def _next_constraint_batch(self, i) -> Mapping:
        return next(self._endless_constraint_iterator[i])
...
# class ConstrainedGAIL(AdversarialTrainer):
#     def __init__(
#         self,
#         venv: vec_env.VecEnv,
#         expert_data: Union[Iterable[Mapping], types.Transitions],
