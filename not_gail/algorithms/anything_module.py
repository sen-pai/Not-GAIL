...
from imitation.rewards import neg_discrim_nets
from imitation.util import logger, reward_wrapper, util

"""
Modifications: Check for the ADDED comment, or UPDATED comment

Discriminator functions are left untouched, pass in a different Disc from neg_discrim_net, everything else is the same

An additional generator: neg_gen_algo, all the same functions of original gen are duplicated. 
"""
...
class AdversarialTrainer:
    ...
    def __init__(
        ...
        neg_gen_algo: on_policy_algorithm.OnPolicyAlgorithm, #ADDED
        discrim: neg_discrim_nets.DiscrimNet, #UPDATED
        expert_data: Union[Iterable[Mapping], types.Transitions], #Only 1 dataset is provided for Anything
        ...
        *,
        ...
        neg_gen_replay_buffer_capacity: Optional[int] = None, #ADDED
        ...
        self.neg_gen_algo = neg_gen_algo #ADDED
        ...
        if debug_use_ground_truth:
            ...
            #ADDED
            self.venv_neg_train = reward_wrapper.RewardVecEnvWrapper(
                self.venv, self.discrim.neg_predict_reward_train
            )
            self.venv_neg_test = reward_wrapper.RewardVecEnvWrapper(
                self.venv, self.discrim.neg_predict_reward_test
            )
        ...
        #ADDED
        self.venv_neg_train_buffering = wrappers.BufferingWrapper(self.venv_neg_train)
        self.venv_neg_train_norm = vec_env.VecNormalize(self.venv_neg_train_buffering)
        self.neg_gen_algo.set_env(self.venv_neg_train_norm)
        ...
        #ADDED
        if neg_gen_replay_buffer_capacity is None:
            neg_gen_replay_buffer_capacity = self.gen_batch_size
        self._neg_gen_replay_buffer = buffer.ReplayBuffer(
            neg_gen_replay_buffer_capacity, self.venv
        )
    ...
    def train_neg_gen(
        ...
        """Trains the neg generator to minimize the discriminator loss.
        I want my (s,a) to be classified as low all the time.
        I dont want to confuse the discriminator at all, it is just a trained classifier for me.
        ...
        """
        if total_timesteps is None:
            # it is fine rn to keep using self.gen_batch_size
            total_timesteps = self.gen_batch_size
        if learn_kwargs is None:
            learn_kwargs = {}

        with logger.accumulate_means("neg_gen"):
            self.neg_gen_algo.learn(
                total_timesteps=total_timesteps,
                reset_num_timesteps=False,
                **learn_kwargs,
            )
        ...
        neg_gen_samples = self.venv_neg_train_norm.pop_transitions()
        self._neg_gen_replay_buffer.store(neg_gen_samples)
    ...
    def train(
        ...
        for r in tqdm.tqdm(range(0, n_rounds), desc="round"):
            ...
            self.train_neg_gen(self.gen_batch_size) #ADDED
...
#ADDED whole class
class AnythingNegGAIL(AdversarialTrainer):
    def __init__(
        self,
        venv: vec_env.VecEnv,
        ...
        expert_batch_size: int,
        gen_algo: on_policy_algorithm.OnPolicyAlgorithm,
        neg_gen_algo: on_policy_algorithm.OnPolicyAlgorithm,
        *,
        discrim_kwargs: Optional[Mapping] = None,
        **kwargs,
    ):
        """Anything Module for Neg GAIL.
        """
        discrim_kwargs = discrim_kwargs or {}
        discrim = neg_discrim_nets.DiscrimNetNegGAIL(
            venv.observation_space, venv.action_space, **discrim_kwargs
        )
        super().__init__(
            venv, gen_algo = gen_algo, neg_gen_algo = neg_gen_algo, discrim =  discrim, expert_data =  expert_data, expert_batch_size =  expert_batch_size, **kwargs
        )