...
if args.load:
    bac_trainer.bac_classifier.load_state_dict(torch.load("bac_weights/"+ args.save_name+".pt", map_location=torch.device('cpu')))
else:
    if args.warm:
...
while x != "n":
    ...
    print(np.array(obs_list).shape, np.array(action_list).shape)
